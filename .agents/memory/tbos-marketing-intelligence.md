...
- GA4 connects via pasted service-account JSON + numeric property id (JWT RS256 self-signed via node:crypto, analyticsdata v1beta runReport); site metrics live in site_daily_metrics keyed (company_id, property_id, date) — company_id in the key is the tenant boundary; sync uses explicit IST calendar dates, never "NdaysAgo" relative ranges.
- Internal dashboard endpoints live under /marketing-intelligence/* (requireSuperAdmin explicitly — the router.use("/ad-connections") guard does NOT cover them); client-role users blocked by the global /client/* guard.
- Client overview "traffic" block: choose lifetime fallback only when NO daily rows exist in range (dailyRows.length===0), never when range sums are zero — zero periods must report zero, not lifetime totals.
